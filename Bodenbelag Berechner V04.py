                    ...
                    if property_set.Name == 'BaseQuantities':
                        for quantity in property_set.Quantities:
                            if quantity.Name == 'NetFloorArea':
                                net_floor_area = round(quantity.AreaValue, 2) if quantity.AreaValue is not None else None
                            elif quantity.Name == 'NetPerimeter':
                                net_perimeter = round(quantity.LengthValue, 2) if quantity.LengthValue is not None else None

        data.append([timestamp_ifc, export_timestamp, file_name, room_number, long_name, net_floor_area, net_perimeter])

    return pd.DataFrame(data, columns=['Timestamp_IFC', 'ExportTimestamp', 'FileName', 'Raumnummer', 'LongName', 'NetFloorArea in m2', 'NetPerimeter in m'])
...
def main():
    ...
    if file_path:
        ...
        output_file = os.path.join(desktop, 'Ausmass_Bodenbelag.xlsx')
